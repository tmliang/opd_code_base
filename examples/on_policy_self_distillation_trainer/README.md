# Code Base for OPD & OPSD 

> **本仓库蒸馏栈一览（先选 OPD 还是 OPSD）**
>
> | 路线 | 教师 | 配置入口 | 适用 | 示例目录 |
> |---|---|---|---|---|
> | **OPD**（On-Policy Distillation，外挂教师） | 独立的、更强的教师模型（多教师亦可） | `distillation.mode=external` + `distillation.teacher_models.*` | 已经有更强的教师 ckpt，愿意付额外 GPU/显存 | [`../on_policy_distillation_trainer/`](../on_policy_distillation_trainer/) |
> | **OPSD**（On-Policy Self-Distillation，自蒸馏） | 同一份模型（colocated ref policy），通过**重写 prompt**激发教师信号 | `distillation.mode=self` + `distillation.self_distill.dataloader=<注册名 或 pkg.module:Class>` | 没有更强教师；想零额外显存就能蒸馏 | **本目录** |
>
> 两条路线共用同一套蒸馏 loss kernel（`distillation.distillation_loss.*`）、同一个 `verl.trainer.main_ppo` 入口。区别只在"教师从哪来"。
>
> - OPD 的详细背景、论文复现、多教师配置：见 [`../on_policy_distillation_trainer/README.md`](../on_policy_distillation_trainer/README.md)（含 OPD / MOPD / Revisiting-OPD / Qwen3-VL OPD 等子配方，Author: [Jacob Helwig](https://jacobhelwig.github.io/)）。
> - OPSD 的所有细节（OPSD / SDPO / Vision-OPD 三个具体配方、teacher dataloader 注册机制、超参速查）见下文。
>
> 配方对照：
>
> | 配方 | 路线 | 启动脚本 |
> |---|---|---|
> | 通用 OPD (FSDP / Megatron / VL) | OPD | [`run_qwen3_8b_fsdp.sh`](../on_policy_distillation_trainer/run_qwen3_8b_fsdp.sh) / [`run_qwen3_8b_megatron.sh`](../on_policy_distillation_trainer/run_qwen3_8b_megatron.sh) / [`run_qwen3_vl_8b_fsdp.sh`](../on_policy_distillation_trainer/run_qwen3_vl_8b_fsdp.sh) |
> | 多教师 OPD (MOPD) | OPD | [`run_qwen3_8b_mopd_fsdp.sh`](../on_policy_distillation_trainer/run_qwen3_8b_mopd_fsdp.sh) |
> | Revisiting-OPD（论文复现，含 5 个 trick 开关） | OPD | [`run_revisiting_opd_fsdp.sh`](../on_policy_distillation_trainer/run_revisiting_opd_fsdp.sh) |
> | OPSD 通用模板 | OPSD | [`../on_policy_distillation_trainer/run_qwen3_8b_opsd_fsdp.sh`](../on_policy_distillation_trainer/run_qwen3_8b_opsd_fsdp.sh) |
> | 原始 OPSD 论文复现 | OPSD | [`run_qwen_opsd.sh`](./run_qwen_opsd.sh) |
> | SDPO（EMA teacher + alpha-KL） | OPSD | [`run_qwen_sdpo.sh`](./run_qwen_sdpo.sh) |
> | Vision-OPD（多模态图像 prompt 改写） | OPSD | [`run_qwen_vision_opd.sh`](./run_qwen_vision_opd.sh) |
...
上述三个脚本用到的 teacher dataloader 已经合入 verl 包，注册名分别为 `opsd` / `sdpo` / `vision_opd`（详见 [verl/workers/self_distillation/dataloaders/](../../verl/workers/self_distillation/dataloaders/)）；Vision-OPD 的 V*Bench MCQ 准确率 reward 也已合入 [verl/utils/reward_score/vision_mcq.py](../../verl/utils/reward_score/vision_mcq.py)，按 `data_source` 自动派发。
...
distillation.self_distill.dataloader=my_pkg.opsd:GoldAnswerTeacher \   # 也可以直接写已注册的短名，如 opsd / sdpo / vision_opd
...
下面三个示例展示了同一套 OPSD 框架如何覆盖 OPSD / SDPO / Vision-OPD。对应实现已经作为内置 dataloader 合入 [`verl/workers/self_distillation/dataloaders/`](../../verl/workers/self_distillation/dataloaders/)，注册名分别为 `opsd` / `sdpo` / `vision_opd`，脚本里直接写 `distillation.self_distill.dataloader=vision_opd` 即可，无需配 `PYTHONPATH`。
...
- **`dataloader`**（str，默认 `null`，**必填**）：teacher dataloader 的注册名（如 `opsd` / `sdpo` / `vision_opd`，见 [verl/workers/self_distillation/dataloaders/](../../verl/workers/self_distillation/dataloaders/)），也支持完整 FQN `pkg.module:Class`。类必须继承 `OfflineTeacherDataloader` 或 `OnlineTeacherDataloader`。
...
| 现象 | 解决 |
|---|---|
| `self_distill.dataloader` 为空 | 必须设置为已注册的短名（`opsd`/`sdpo`/`vision_opd` 等）或 `pkg.module:Class` FQN |
